...
| 0.2.522 / qf 0.1.567 / build 0.1.135 / note 622 | (CODEX) POV 6.09.03 navigation timing restored and the metadata-timeout repairs preserved after the Windows patchers; Umbrella's watched-sync now rejects malformed pages, freezes the server checkpoint before a forced refresh and backfills skipped history once -- so `HOST_FIXED_IN` was deliberately NOT declared for the Umbrella patcher: 6.7.87's own `if data is None` guard misses get_request's `{}` fallback after a JSON decode failure |
| 0.2.523 / qf 0.1.568 / build 0.1.136 / note 623 | the Arabic gender reference could not read Arabic -- every pattern needed harakat and real subtitles carry none (7,415 letters, 2 diacritics in a full episode), so it returned F=0 M=0 None=370 while anchoring 41% of all field jobs; it now reads the consonantal 2fs present suffix, refusing the tafil verbal noun (five letters, indistinguishable from a real five-letter verb, so the floor is six) and the feminine sound dual. Also: `_HE_REF_FEM` gains the proclitics `_HE_MASC` always had, so the reader is no longer better at finding a MALE addressee than a female one; `begin()` actually never raises as its docstring promised; two false docstrings corrected; and two tests red for a POV layout the patchers handle fine now ask the tree |
| 0.2.524 / qf 0.1.569 / build 0.1.137 / note 624 | a false feminine verdict did not degrade gracefully -- it REWROTE a correct line: `wrong_gender_entries` flags the entry and `_regender_blocks` tells the model "these address a FEMALE listener" with no veto but "the rewrite no longer says `אתה`". `את` is also the definite-object marker and definiteness is an OPEN class, so extending the exception list can never finish; reading 2fs verb MORPHOLOGY instead took a full human episode from 57 to 69 verdicts with the 13 object-marker false positives gone and 23 genuine imperatives gained -- precision and recall both improved. Arabic verbs carrying an object clitic are read too, and the one false-positive class that opens (broken plural + possessive) is pinned as a test |
| 0.2.525 / qf 0.1.570 / build 0.1.138 / note 625 | the final subtitle swap marked itself successful because `setSubtitles()` had not raised -- but it POSTS to the video-player thread, so the stream list read next was the PRE-add list and "pick the most recent stream" picked the last progressive SLOT, which the cleanup then deleted: viewer pinned to a stream with no file, no subtitles for the rest of playback. It now waits for the stream count to actually grow, keeps the slots when it does not, treats "no player" as safe-to-clean, and `glob.escape()`s release names (brackets are kept on purpose, so `[YTS.MX]` read as a character class made the cleanup a silent no-op). A THIRD success path -- the Google rescue when model output is not Hebrew -- emitted no `done` at all. A `.google`/`.shared` marker could be evicted while its translation stayed, after which machine translation could reach the community pool. Two picker labels advertised 250/day for models capped at 20. FOUR validator rounds, seven blockers, two of them fixes of mine that would have been worse; the tier-agnostic cache lookup was CUT because `resolve()`'s early return fires before anything is listening |
| 0.2.526 / qf 0.1.571 / build 0.1.139 / note 626 | "share my cached translations" uploaded NOTHING: `pool.py:881` parsed cached filenames with no slot for the quality tier, and a translation that used a gender reference is `<key>.ar.he.srt` -- the normal case -- so every file counted as `skipped` and the action reported success having sent nothing. The tier is now read from the FILENAME, never from the setting, so a boosted translation is recorded as one and a plain one is not recorded as boosted. Shipping it needed a THIRD packaging path: `pool.py` carries the pool credential, inheritance takes the file whole and would have discarded the fix, and `pkgkey` is the maintainer's only. `$POOL_CARRY_BLOCK_FROM` copies the credential BLOCK byte-for-byte and proves both halves before writing; the quickfix guard was widened the same way -- from "pool.py may not change" to "if it changed, the credential must have survived", placeholder still refused instantly. All five shipped packages verified credential-identical to live 0.2.525, and the repo's own pool.py still holds the placeholder |
...
surfaced. Three rounds of validation; the first two each found something real.

**The regression.** 0.2.499 fixed Umbrella's "Shows in Progress" by clearing
the MDBList sync cursor. It cleared THREE keys. Only one of them is a cursor:
`sync_watchedProgress` reads `last_watched_at` to build `since`
(`modules/mdblist.py:945`). The other two -- `last_watched_movies_at`,
`last_watched_episodes_at` -- are read somewhere else entirely, by
`getMoviesWatchedActivity()` / `getEpisodesWatchedActivity()`
(`mdblist.py:923-931`), and `playcount.py:98` uses them as the "is there new
watched activity" signal:

    elif mdblist.getEpisodesWatchedActivity() < mdblist.timeoutsyncTVShows(): timeout = 720
    else: timeout = 0

`last_sync()` returns 0 for a row that is not there, so deleting those keys
pinned that comparison permanently true: serve the 12-hour cache, never
re-sync. The episodes list -- the one that had always been right -- started
needing a manual refresh too. One stale list became two.

**And my test asserted the bug.** It had a check literally named "the three
cleared keys are the three sync_watchedProgress writes". That is the third time
this project has found a test encoding a defect as the passing condition, and
the first time it was one of ours. The check is now the opposite invariant, by
name: the ACTIVITY signals must be LEFT ALONE.

**The root cause, which 599 only bounded.** `sync_watchedProgress` writes the
cursor to `datetime.utcnow()` unconditionally, including on the path where the
loop left via `if not data: break` -- which is the FAILURE path. The cursor
only moves forward, so a window inside a failed request was skipped forever.
599 widened the fetch window 30 days, which bounds the loss; it does not
prevent it. 601 makes the three `update_last_watched_at()` calls conditional on
the fetch having actually returned a page.

**"Is it a page", not "is it empty" -- round 1's finding.** The guard first
read `if not data:`, which catches the two shapes `get_request()` is *written*
to produce on failure: `None` on any error, `{}` on a 2xx whose body will not
parse (`mdblist.py:868-878`). But it returns `response.json()` VERBATIM, so a
2xx carrying a soft-fail envelope is a truthy dict: the loop ingests nothing,
finds no `pagination`, and exits as though it had reached the last page. Stock
has the same blind spot. The test is now positive --
`not isinstance(data, dict) or 'pagination' not in data` -- and the cost
asymmetry is why: a response with no `pagination` cannot paginate anyway (the
loop reads one page and breaks), so a wrongly-frozen cursor costs ONE extra
request per sync interval against `INSERT OR REPLACE` upserts. The opposite
mistake is the bug.

**Four lines, all or none, all pure insertions.** Three of them only work as a
set, and a partial application would inject a `NameError` into Umbrella inside
a bare `except: log_utils.error()` that would swallow it and take the whole
sync down. Each anchor must also appear EXACTLY once, so a refactor that
duplicated a shape is treated as unrecognised rather than patched at whichever
copy comes first. And every injection goes BETWEEN two existing lines, never
over one: wrapping the three update calls in an `if` and re-indenting them
reads better and is wrong, because `_revert()` deletes a marked line plus
everything indented under it and would take Umbrella's own three lines with it.

**POV does NOT have this bug, and now I know why.** POV has no cursor at all.
It re-fetches the whole watched list and does a delete-and-replace
(`caches/mdbl_cache.py:25-33`). Nothing is incremental, so nothing can be
skipped forever -- which is exactly why POV showed the right progress
throughout. It does have a RELATED defect, recorded below and not fixed here.

**A test that reported FAIL on every machine but this one -- round 2.**
`test_mdblist_search_nav.py` returned ten failures wherever no stock POV tree
exists: its inline fixtures were hand-written miniatures missing shapes the
patcher anchors on, and two sections built their tree with
`STOCK if os.path.isdir(STOCK) else root` -- where `root` is the tree that same
file patched three hundred lines earlier, so the v2 upgrade replay met a device
already carrying the current marker. Fixtures are now generated byte-slices of
real POV 6.08.13, and one `lay_down_pristine()` serves all three sites. A test
that proves nothing off one machine is a defect here; a test that reports RED
off one machine is worse, because it teaches people to scroll past red.

**Source citations are now pinned.** Round 2 also found `playcount.py:97` for a
line at 98. These citations are the only reason a later reader trusts the
reasoning instead of re-deriving it, so every `<file>.py:<lines>` reference in
the patcher is checked against a required fragment at the cited range; an
unpinned citation fails, and a pin with no citation fails too. It caught a
stale pin of my own on its first run. Where there is no stock tree to check
against, the suite says so in a line of its own rather than passing quietly.

**The standalone was about to announce a fix it does not carry.** Found while
verifying the artifacts, not by a test. The standalone package is "AI subtitles
+ DarkSubs/OpenSubtitles only" -- no `pov_*` files, no Umbrella patcher, and it
runs `SLIM_SERVICE`, which does not run the build's startup repairs at all. Its
changelog is the shared one, filtered by `slim_changelog_text()`. That filter is
a blocklist, and there were TWO of them: the filter's own tuple, and a shorter
HAND-COPY inside `assert_no_standalone_build_payload()`. A term in neither
passed both, and three did -- `Umbrella`, `MDBList`, `POV` -- so ten bullets
across fourteen releases told standalone users about fixes to add-ons they do
not have.

That is **note 598 wearing different clothes**: the announcement was shared,
the change was not. Now one list, read by the assertion too, and
`tools/test_standalone_changelog_scope.py` derives which hosts the standalone
patches from `include_standalone()` itself rather than taking a second list on
trust -- so hiding a fix those users DID receive fails as loudly as announcing
one they did not. Consequence handled rather than shipped: with the hosts
...
returns the old), and `_get_mdbl_paginated_list` swallows a failure into empty
lists, which `mdbl_indicators_movies/_tv` turn into a full table DELETE. One
failed request right after a watch event wipes every watched tick, and nothing
re-fetches until MDBList reports new activity. Self-healing on the next watch,
unlike Umbrella's, which was permanent -- so a different class, and left for
its own round. It is not a one-liner: POV returns the same value for "the
request failed" and "this account has watched nothing", and suppressing a
LEGITIMATE wipe would leave stale ticks forever, which is worse.
...
39 feature tests pass, with independent read-only review and build/standalone staging checks. Standalone packaging excludes the feature and strips its dispatch route. No version/manifests/public notes have been bumped. Remaining release gates: live four-skin/remote behavior, favourites cache/skin-switch behavior, actual catalog latency/cancellation and recommendation usefulness. Do not describe this as shipped or fully validated.

Provider integration update: the feature shares the home search_provider setting for POV/Umbrella catalog, recommendation and playback routing, rejects stale results after a provider switch, and preserves feedback by media type/TMDB identity. Remote Umbrella indicators use positive native catalog markers only; missing remote history is unknown, never silently replaced with local history. Temporary similar/different/shorter refinements and explicit provider-native trailer/extras actions are available. Independent review caught and verified fixes for numeric Umbrella resume duration metadata and consistent anchored refresh from saved/introduction entries. Thirteen additional private offline probes passed; this does not certify live Kodi.

Translation detector: narrow lexical exclusions prevent two Arabic noun families with possessive suffixes from masquerading as feminine addressee evidence. Other gender evidence in the same cue remains active. Regression tests and an independent in-memory mutation establish the narrow fix; this is not a full Arabic morphological parser or a general translation accuracy claim. No release versions changed.


Further local development (not released): evening onboarding can search a known liked title through the active provider or collect several explicit likes at once. The compact main menu groups onboarding and time/media choices; own feedback/saved/seen markers can be removed and up to five current-session preference changes undone without reverting the provider catalog. TV-only plus a new time limit now explains the switch to movies rather than silently returning no results. 48 feature tests, including mocked complete menu flows, pass.

The gender repair now explicitly opts into both reference directions, carries a per-entry suggested target, declines mixed output evidence, and checks the corresponding opposite form before accepting a replacement. Five new extracted-code regression tests exercise mixed-target batches, actual production wiring, KEEP, structural constraints, numbers and negation. Existing gender, translation-safety and embedded-chain suites pass. Small live fixtures support feasibility; they do not establish population accuracy. Source-lookahead remains a private experiment and has not been enabled in production. No versions, packages in release distribution, manifests or public update notes changed.
